...
class SignalGenerator:
    ...
    def _generate_critical(self, snap: IntelSnapshot, q: TradeQueue) -> None:
        ...
        if snap.mass_liquidation:
            ...
            if side in ("long", "short"):
                self._propose(q, TradeProposal(
                    priority=SignalPriority.CRITICAL,
                    symbol="BTC/USDT",
                    side=side,
                    strategy="liq_reversal",
                    reason=f"Mass liq ${snap.liquidation_24h/1e9:.1f}B — "
                           f"bias {side} (exhaustion reversal)",
                    strength=0.85,
                    leverage=10,
                    quick_trade=True,
                    max_hold_minutes=15,
    ...
    def _generate_swing(self, snap: IntelSnapshot, q: TradeQueue) -> None:
        ...
        if snap.regime in ("risk_off", "caution") and snap.fear_greed >= 70:
            self._propose(q, TradeProposal(
                ...
                reason=f"Risk-off regime + F&G={snap.fear_greed} (greed) — "
                       f"planned short if momentum breaks",
                strength=0.6,
                leverage=5,
        ...
        if btc_tv and eth_tv:
            ...
            if btc_strong and eth_weak and snap.fear_greed < 60:
                self._propose(q, TradeProposal(
                    ...
                    reason="BTC strong, ETH lagging — rotation play when "
                           "ETH catches up",
                    strength=0.55,
                    leverage=5,
    ...
    def _propose(self, queue: TradeQueue, proposal: TradeProposal) -> None:
        ...
        if key in self._recent_ids:
            ...
            elapsed = (datetime.now(timezone.utc) - self._recent_ids[key]).total_seconds()
            if elapsed < cooldown:
                return
        ...
        queue.add(proposal)
        self._recent_ids[key] = datetime.now(timezone.utc)
        logger.info("QUEUE [{}] {} {} — {} (str={:.2f})",
                     proposal.priority.value.upper(),
                     proposal.side.upper(), proposal.symbol,
                     proposal.reason, proposal.strength)

    def _purge_cooldowns(self) -> None:
        cutoff = datetime.now(timezone.utc) - timedelta(hours=24)
        self._recent_ids = {
            k: v for k, v in self._recent_ids.items() if v > cutoff
        }
