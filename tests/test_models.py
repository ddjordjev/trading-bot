...
from core.models.market import Candle, Ticker, OrderBook, MarketType
...
class TestCandle:
    def _make(self, **kw) -> Candle:
        defaults = dict(timestamp=datetime.now(timezone.utc), open=100.0, high=110.0,
...
class TestTicker:
    def _make(self, **kw) -> Ticker:
        defaults = dict(symbol="BTC/USDT", bid=100.0, ask=101.0, last=100.5,
...
class TestOrderBook:
    def test_creation(self):
        ob = OrderBook(symbol="BTC/USDT", bids=[(100, 1)], asks=[(101, 1)],
                       timestamp=datetime.now(timezone.utc))
...
class TestOrder:
    def test_remaining(self):
        o = Order(symbol="BTC/USDT", side=OrderSide.BUY, order_type=OrderType.MARKET,
                  amount=10.0, filled=3.0)
        assert o.remaining == pytest.approx(7.0)

    def test_is_complete_filled(self):
        o = Order(symbol="BTC/USDT", side=OrderSide.BUY, order_type=OrderType.MARKET,
                  amount=10.0, status=OrderStatus.FILLED)
        assert o.is_complete is True

    def test_is_complete_cancelled(self):
        o = Order(symbol="BTC/USDT", side=OrderSide.BUY, order_type=OrderType.MARKET,
                  amount=10.0, status=OrderStatus.CANCELLED)
        assert o.is_complete is True

    def test_is_not_complete_pending(self):
        o = Order(symbol="BTC/USDT", side=OrderSide.BUY, order_type=OrderType.MARKET,
                  amount=10.0, status=OrderStatus.PENDING)
...
class TestPosition:
    def test_notional_value(self):
        p = Position(symbol="BTC/USDT", side=OrderSide.BUY, amount=2.0,
                     entry_price=100, current_price=110)
        assert p.notional_value == pytest.approx(220.0)

    def test_pnl_pct_long(self):
        p = Position(symbol="BTC/USDT", side=OrderSide.BUY, amount=1.0,
                     entry_price=100, current_price=110, leverage=10)
        assert p.pnl_pct == pytest.approx(100.0)

    def test_pnl_pct_short(self):
        p = Position(symbol="BTC/USDT", side=OrderSide.SELL, amount=1.0,
                     entry_price=100, current_price=90, leverage=10)
        assert p.pnl_pct == pytest.approx(100.0)

    def test_pnl_pct_zero_entry(self):
        p = Position(symbol="BTC/USDT", side=OrderSide.BUY, amount=1.0,
                     entry_price=0, current_price=110)
...
class TestSignal:
    ...
    def test_quick_trade(self):
        s = Signal(symbol="BTC/USDT", action=SignalAction.BUY, quick_trade=True,
                   max_hold_minutes=5)
