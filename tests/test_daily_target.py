...
class TestUpdateBalance:
    """update_balance simply stores the current balance."""

    def test_update_balance_stores_value(self):
        t = DailyTargetTracker(daily_target_pct=10.0)
        t.reset_day(1000.0)
        t.update_balance(1050.0)
        assert t._current_balance == pytest.approx(1050.0)
        assert t.todays_pnl_pct == pytest.approx(5.0)

    def test_total_growth_simple(self):
        t = DailyTargetTracker(daily_target_pct=10.0)
        t.reset_day(1000.0)
        t.update_balance(1100.0)
        assert t.total_growth_pct == pytest.approx(10.0)
