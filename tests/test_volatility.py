...
def _ticker(symbol="BTC/USDT", last=100.0, ts=None) -> Ticker:
    return Ticker(symbol=symbol, bid=last - 0.5, ask=last + 0.5, last=last,
                  volume_24h=1e6, change_pct_24h=1.0,
                  timestamp=ts or datetime.now(timezone.utc))


class TestSpikeEvent:
    def test_creation(self):
        e = SpikeEvent(symbol="BTC/USDT", direction="up", change_pct=5.0,
                       price=105, volume_24h=1e6, window_seconds=60)
